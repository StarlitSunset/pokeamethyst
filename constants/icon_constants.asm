; IconPointers indexes (see data/icon_pointers.asm)
	const_def
	const ICON_NULL
	const ICON_HANAGOMURA  ; HANAGOMURA
	const ICON_HONOOGUMA   ; HONOOGUMA
	const ICON_BORUBEAA    ; BORUBEAA
	const ICON_DAINABEA    ; DAINABEA
	const ICON_KURUSU      ; KURUSU
	const ICON_AKUA		   ; AKUA
	const ICON_AKUERIA     ; AKUERIA
	const ICON_MIKON	   ; MIKON
	const ICON_MONJA  	   ; MONJA
	const ICON_MANBOO      ; MANBOO
	const ICON_IKARI       ; IKARI
	const ICON_BUTTERFREE  ; BUTTERFREE
	const ICON_GURUTESU    ; GURUTESU
	const ICON_PARA        ; PARA 
	const ICON_ANIMON      ; ANIMON
	const ICON_PIDGEY      ; PIDGEY
	const ICON_PIDGEOTTO   ; PIDGEOTTO
	const ICON_PIDGEOT     ; PIDGEOT
	const ICON_SHIBIFUGU   ; SHIBIFUGU
	const ICON_GYOPIN      ; GYOPIN
	const ICON_JARANRA     ; JARANRA
	const ICON_HANEEI      ; HANEEI
	const ICON_HINAAZU     ; HINAAZU
	const ICON_TSUINZU     ; TSUINZU
	const ICON_PIKACHU     ; PIKACHU
	const ICON_RAICHU      ; RAICHU
	const ICON_SANDSHREW   ; SANDSHREW
	const ICON_SANDSLASH   ; SANDSLASH
	const ICON_RINRIN      ; RINRIN
	const ICON_BERURUN     ; BERURUN
	const ICON_NYOROTONO   ; NYOROTONO
	const ICON_MITSUBOSHI  ; MITSUBOSHI
	const ICON_PUCHIKOON   ; PUCHIKOON
	const ICON_TAABAN      ; TAABAN
	const ICON_CLEFAIRY    ; CLEFAIRY
	const ICON_CLEFABLE    ; CLEFABLE
	const ICON_VULPIX      ; VULPIX
	const ICON_NINETALES   ; NINETALES
	const ICON_JIGGLYPUFF  ; JIGGLYPUFF
	const ICON_WIGGLYTUFF  ; WIGGLYTUFF
	const ICON_ZUBAT       ; ZUBAT
	const ICON_GOLBAT      ; GOLBAT
	const ICON_ODDISH      ; ODDISH
	const ICON_GLOOM       ; GLOOM
	const ICON_VILEPLUME   ; VILEPLUME
	const ICON_PARAS       ; PARAS
	const ICON_PARASECT    ; PARASECT
	const ICON_BETOBEBII   ; BETOBEBII
	const ICON_TEPPOUO     ; TEPPOUO
	const ICON_WATAKKO     ; WATAKKO
	const ICON_BARIRIINA   ; BARIRIINA
	const ICON_MEOWTH      ; MEOWTH
	const ICON_PERSIAN     ; PERSIAN
	const ICON_KOTORA      ; KOTORA
	const ICON_RAITORA     ; RAITORA
	const ICON_TSUBOMITTO  ; TSUBOMITTO
	const ICON_BOMUSHIKAA  ; BOMUSHIKAA
	const ICON_GROWLITHE   ; GROWLITHE
	const ICON_ARCANINE    ; ARCANINE
	const ICON_POLIWAG     ; POLIWAG
	const ICON_POLIWHIRL   ; POLIWHIRL
	const ICON_POLIWRATH   ; POLIWRATH
	const ICON_ABRA        ; ABRA
	const ICON_KADABRA     ; KADABRA
	const ICON_ALAKAZAM    ; ALAKAZAM
	const ICON_MADAAMU     ; MADAAMU
	const ICON_MACHOKE     ; MACHOKE*
	const ICON_MACHAMP     ; MACHAMP*
	const ICON_BELLSPROUT  ; BELLSPROUT
	const ICON_WEEPINBELL  ; WEEPINBELL
	const ICON_VICTREEBEL  ; VICTREEBEL
	const ICON_TENTACOOL   ; TENTACOOL
	const ICON_TENTACRUEL  ; TENTACRUEL
	const ICON_GEODUDE     ; GEODUDE
	const ICON_GRAVELER    ; GRAVELER
	const ICON_GOLEM       ; GOLEM
	const ICON_PONYTA      ; PONYTA
	const ICON_RAPIDASH    ; RAPIDASH
	const ICON_SLOWPOKE    ; SLOWPOKE
	const ICON_SLOWBRO     ; SLOWBRO
	const ICON_PUDI        ; PUDI
	const ICON_HANEKKO     ; HANEKKO
	const ICON_FARFETCH_D  ; FARFETCH_D
	const ICON_DODUO       ; DODUO
	const ICON_DODRIO      ; DODRIO
	const ICON_SEEL        ; SEEL*
	const ICON_DEWGONG     ; DEWGONG*
	const ICON_GRIMER      ; GRIMER
	const ICON_MUK         ; MUK
	const ICON_SHELLDER    ; SHELLDER*
	const ICON_CLOYSTER    ; CLOYSTER*
	const ICON_GASTLY      ; GASTLY
	const ICON_HAUNTER     ; HAUNTER
	const ICON_GENGAR      ; GENGAR
	const ICON_ONIX        ; ONIX
	const ICON_KIRINRIKI   ; KIRINRIKI
	const ICON_KOONYA      ; KOONYA
	const ICON_KRABBY      ; KRABBY*
	const ICON_KINGLER     ; KINGLER*
	const ICON_POPOKKO     ; POPOKKO
	const ICON_ELECTRODE   ; ELECTRODE
	const ICON_EXEGGCUTE   ; EXEGGCUTE
	const ICON_EXEGGUTOR   ; EXEGGUTOR
	const ICON_CUBONE      ; CUBONE
	const ICON_MAROWAK     ; MAROWAK
	const ICON_HITMONLEE   ; HITMONLEE
	const ICON_HITMONCHAN  ; HITMONCHAN
	const ICON_LICKITUNG   ; LICKITUNG
	const ICON_OKUTANK     ; OKUTANK
	const ICON_BARUKII     ; BARUKII
	const ICON_RHYHORN     ; RHYHORN
	const ICON_RHYDON      ; RHYDON
	const ICON_CHANSEY     ; CHANSEY
	const ICON_TANGELA     ; TANGELA
	const ICON_KANGASKHAN  ; KANGASKHAN*
	const ICON_HORSEA      ; HORSEA
	const ICON_SEADRA      ; SEADRA
	const ICON_GOLDEEN     ; GOLDEEN
	const ICON_SEAKING     ; SEAKING
	const ICON_STARYU      ; STARYU*
	const ICON_STARMIE     ; STARMIE*
	const ICON_MR__MIME    ; MR__MIME
	const ICON_SCYTHER     ; SCYTHER
	const ICON_JYNX        ; JYNX
	const ICON_ELECTABUZZ  ; ELECTABUZZ
	const ICON_MAGMAR      ; MAGMAR
	const ICON_PINSIR      ; PINSIR
	const ICON_TAUROS      ; TAUROS
	const ICON_MAGIKARP    ; MAGIKARP
	const ICON_GYARADOS    ; GYARADOS
	const ICON_LAPRAS      ; LAPRAS
	const ICON_DITTO       ; DITTO
	const ICON_EEVEE       ; EEVEE
	const ICON_VAPOREON    ; VAPOREON
	const ICON_JOLTEON     ; JOLTEON
	const ICON_FLAREON     ; FLAREON
	const ICON_PORYGON     ; PORYGON
	const ICON_OMANYTE     ; OMANYTE*
	const ICON_OMASTAR     ; OMASTAR*
	const ICON_KABUTO      ; KABUTO*
	const ICON_KABUTOPS    ; KABUTOPS*
	const ICON_AERODACTYL  ; AERODACTYL
	const ICON_SNORLAX     ; SNORLAX
	const ICON_ARTICUNO    ; ARTICUNO
	const ICON_ZAPDOS      ; ZAPDOS
	const ICON_MOLTRES     ; MOLTRES
	const ICON_DRATINI     ; DRATINI
	const ICON_DRAGONAIR   ; DRAGONAIR
	const ICON_DRAGONITE   ; DRAGONITE
	const ICON_MEWTWO      ; MEWTWO
	const ICON_MEW         ; MEW
	const ICON_CHIKORITA   ; CHIKORITA
	const ICON_BAYLEEF     ; BAYLEEF
	const ICON_MEGANIUM    ; MEGANIUM
	const ICON_CYNDAQUIL   ; CYNDAQUIL
	const ICON_QUILAVA     ; QUILAVA
	const ICON_TYPHLOSION  ; TYPHLOSION
	const ICON_TOTODILE    ; TOTODILE
	const ICON_CROCONAW    ; CROCONAW
	const ICON_FERALIGATR  ; FERALIGATR
	const ICON_SENTRET     ; SENTRET
	const ICON_FURRET      ; FURRET
	const ICON_HOOTHOOT    ; HOOTHOOT
	const ICON_NOCTOWL     ; NOCTOWL
	const ICON_LEDYBA      ; LEDYBA
	const ICON_LEDIAN      ; LEDIAN
	const ICON_SPINARAK    ; SPINARAK
	const ICON_ARIADOS     ; ARIADOS
	const ICON_CROBAT      ; CROBAT
	const ICON_CHINCHOU    ; CHINCHOU
	const ICON_LANTURN     ; LANTURN
	const ICON_PICHU       ; PICHU
	const ICON_CLEFFA      ; CLEFFA
	const ICON_IGGLYBUFF   ; IGGLYBUFF
	const ICON_TOGEPI      ; TOGEPI
	const ICON_TOGETIC     ; TOGETIC
	const ICON_NATU        ; NATU
	const ICON_XATU        ; XATU
	const ICON_MAREEP      ; MAREEP
	const ICON_FLAAFFY     ; FLAAFFY
	const ICON_AMPHAROS    ; AMPHAROS
	const ICON_BELLOSSOM   ; BELLOSSOM
	const ICON_MARILL      ; MARILL
	const ICON_AZUMARILL   ; AZUMARILL
	const ICON_SUDOWOODO   ; SUDOWOODO
	const ICON_POLITOED    ; POLITOED
	const ICON_HOPPIP      ; HOPPIP
	const ICON_SKIPLOOM    ; SKIPLOOM
	const ICON_JUMPLUFF    ; JUMPLUFF
	const ICON_AIPOM       ; AIPOM
	const ICON_SUNKERN     ; SUNKERN
	const ICON_SUNFLORA    ; SUNFLORA
	const ICON_YANMA       ; YANMA
	const ICON_WOOPER      ; WOOPER
	const ICON_QUAGSIRE    ; QUAGSIRE
	const ICON_ESPEON      ; ESPEON
	const ICON_UMBREON     ; UMBREON
	const ICON_MURKROW     ; MURKROW
	const ICON_SLOWKING    ; SLOWKING
	const ICON_MISDREAVUS  ; MISDREAVUS
	const ICON_UNOWN       ; UNOWN
	const ICON_WOBBUFFET   ; WOBBUFFET
	const ICON_GIRAFARIG   ; GIRAFARIG
	const ICON_PINECO      ; PINECO
	const ICON_FORRETRESS  ; FORRETRESS
	const ICON_DUNSPARCE   ; DUNSPARCE
	const ICON_GLIGAR      ; GLIGAR
	const ICON_STEELIX     ; STEELIX
	const ICON_SNUBBULL    ; SNUBBULL
	const ICON_GRANBULL    ; GRANBULL
	const ICON_QWILFISH    ; QWILFISH
	const ICON_SCIZOR      ; SCIZOR
	const ICON_SHUCKLE     ; SHUCKLE
	const ICON_HERACROSS   ; HERACROSS
	const ICON_SNEASEL     ; SNEASEL
	const ICON_TEDDIURSA   ; TEDDIURSA
	const ICON_URSARING    ; URSARING
	const ICON_SLUGMA      ; SLUGMA
	const ICON_MAGCARGO    ; MAGCARGO
	const ICON_SWINUB      ; SWINUB
	const ICON_PILOSWINE   ; PILOSWINE
	const ICON_CORSOLA     ; CORSOLA
	const ICON_REMORAID    ; REMORAID
	const ICON_OCTILLERY   ; OCTILLERY
	const ICON_DELIBIRD    ; DELIBIRD
	const ICON_MANTINE     ; MANTINE
	const ICON_SKARMORY    ; SKARMORY
	const ICON_HOUNDOUR    ; HOUNDOUR
	const ICON_HOUNDOOM    ; HOUNDOOM
	const ICON_KINGDRA     ; KINGDRA
	const ICON_PHANPY      ; PHANPY
	const ICON_DONPHAN     ; DONPHAN
	const ICON_PORYGON2    ; PORYGON2
	const ICON_STANTLER    ; STANTLER
	const ICON_SMEARGLE    ; SMEARGLE
	const ICON_TYROGUE     ; TYROGUE
	const ICON_HITMONTOP   ; HITMONTOP
	const ICON_SMOOCHUM    ; SMOOCHUM
	const ICON_ELEKID      ; ELEKID
	const ICON_MAGBY       ; MAGBY
	const ICON_MILTANK     ; MILTANK
	const ICON_BLISSEY     ; BLISSEY
	const ICON_RAIKOU      ; RAIKOU
	const ICON_ENTEI       ; ENTEI
	const ICON_SUICUNE     ; SUICUNE
	const ICON_LARVITAR    ; LARVITAR
	const ICON_PUPITAR     ; PUPITAR
	const ICON_TYRANITAR   ; TYRANITAR
	const ICON_LUGIA       ; LUGIA
	const ICON_HO_OH       ; HO_OH
	const ICON_CELEBI      ; CELEBI
	const ICON_SPICHU	   ; S_PICHU
	const ICON_EGG		   ; EGG
DEF NUM_ICONS EQU const_value - 1

; LoadMenuMonIcon.Jumptable indexes (see engine/gfx/mon_icons.asm)
	const_def
	const MONICON_PARTYMENU
	const MONICON_NAMINGSCREEN
	const MONICON_MOVES
	const MONICON_TRADE
	const MONICON_MOBILE1
	const MONICON_MOBILE2
	const MONICON_UNUSED
	
; party menu icon palettes
	const_def
	const PAL_ICON_RED    ; 0
	const PAL_ICON_BLUE   ; 1
	const PAL_ICON_GREEN  ; 2
	const PAL_ICON_BROWN  ; 3
	const PAL_ICON_PINK   ; 4
	const PAL_ICON_GRAY   ; 5
	const PAL_ICON_TEAL   ; 6
	const PAL_ICON_PURPLE ; 7
