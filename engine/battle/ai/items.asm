...
AI_TryItem:
	...
	ld c, l
	ld hl, AI_Items
; (fixed) BUG: AI might use its base reward value as an item (see docs/bugs_and_glitches.md)
.loop
...
; (fixed) BUG: AI use of Full Heal does not cure confusion status (see docs/bugs_and_glitches.md)
	call AI_HealStatus
	ld a, FULL_RESTORE
	ld [wCurEnemyItem], a
	; fallthrough
...
AI_HealStatus:
; (fixed) BUG: AI use of Full Heal or Full Restore does not cure Nightmare status (see docs/bugs_and_glitches.md)
	ld a, [wCurOTMon]
	ld hl, wOTPartyMon1Status
	ld bc, PARTYMON_STRUCT_LENGTH
	call AddNTimes
