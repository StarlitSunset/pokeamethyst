...
; (fixed) BUG: SFX_RUN does not play correctly when a wild Pokémon flees from battle (see docs/bugs_and_glitches.md)
	ld de, SFX_RUN
	call WaitPlaySFX
...
; (fixed) BUG: Perish Song and Spikes can leave a Pokemon with 0 HP and not faint (see docs/bugs_and_glitches.md)
.faint_loop
	call .Function
	ret c
...
.go
	...
	xor a
	ld [hl], a
; (fixed) BUG: Berserk Gene's confusion lasts for 256 turns or the previous Pokémon's confusion count (see docs/bugs_and_glitches.md)
...
; (fixed) BUG: A Pokémon that fainted from Pursuit will have its old status condition when revived (see docs/bugs_and_glitches.md)
	ld a, $f0
	ld [wCryTracks], a
	ld a, [wBattleMonSpecies]
	call PlayStereoCry
...
.done
; (fixed) BUG: A Disabled but PP Up–enhanced move may not trigger Struggle (see docs/bugs_and_glitches.md)
	and PP_MASK
	ret nz
...
; (fixed) BUG: Magikarp length limits have a unit conversion error (see docs/bugs_and_glitches.md)
	ld de, wEnemyMonDVs
	ld bc, wPlayerID
...
.CheckMagikarpArea:
; (fixed) BUG: Magikarp in Lake of Rage are shorter, not longer (see docs/bugs_and_glitches.md)
	ld a, [wMapGroup]
...
; (fixed) BUG: PRZ and BRN stat reductions don't apply to switched Pokémon (see docs/bugs_and_glitches.md)
	call ApplyStatusEffectOnEnemyStats
	ret
...
; Swap badges 3 (PlainBadge) and 5 (MineralBadge).
	...
	ld c, 4
.CheckBadge:
; (fixed) BUG: Glacier Badge may not boost Special Defense depending on the value of Special Attack (see docs/bugs_and_glitches.md)
	ld a, b
	srl b
	push af
	call c, BoostStat
...
; (fixed) BUG: Switching out or switching against a Pokémon with max HP below 4 freezes the game (see docs/bugs_and_glitches.md)
