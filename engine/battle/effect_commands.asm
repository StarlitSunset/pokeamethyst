...
.get_type
	...
	; fallthrough
CheckTypeMatchup:
; (fixed) BUG: AI makes a false assumption about CheckTypeMatchup (see docs/bugs_and_glitches.md)
	push hl
	push de
	push bc
	ld d, a
...
.got_move_chance
; (fixed) BUG: Moves with a 100% secondary effect chance will not trigger it in 1/256 uses (see docs/bugs_and_glitches.md)
	ld a, [wLinkMode]
	cp LINK_COLOSSEUM
...
; (fixed) BUG: Beat Up may fail to raise Substitute (see docs/bugs_and_glitches.md)
	cp EFFECT_MULTI_HIT
	jr z, .multihit
	cp EFFECT_DOUBLE_HIT
	jr z, .multihit
...
; (fixed) BUG: Metal Powder can increase damage taken with boosted (Special) Defense (see docs/bugs_and_glitches.md)
	ld h, b
	ld l, c
	srl b
	rr c
...
.finish
; (fixed) BUG: Reflect and Light Screen can make (Special) Defense wrap around above 1024 (see docs/bugs_and_glitches.md)
...
; Double the stat
; (fixed) BUG: Thick Club and Light Ball can make (Special) Attack wrap around above 1024 (see docs/bugs_and_glitches.md)
	sla l
	rl h
...
; (fixed) BUG: Confusion damage is affected by type-boosting items and Explosion/Self-Destruct doubling (see docs/bugs_and_glitches.md)
...
.only_one_beatup
; (fixed) BUG: Beat Up works incorrectly with only one Pokémon in the party (see docs/bugs_and_glitches.md)
	ld a, BATTLE_VARS_SUBSTATUS3
	call GetBattleVarAddr
...
CheckHiddenOpponent:
; (fixed) BUG: Lock-On and Mind Reader don't always bypass Fly and Dig (see docs/bugs_and_glitches.md)
	ld a, BATTLE_VARS_SUBSTATUS5_OPP
	call GetBattleVar
	cpl
